...
## Importacao portatil r15

O servidor não possui Python. O r15 inclui Python 3.8.10 embeddable x64 em `C:\Program Files\VISEP\runtime\python`, sem alterar PATH ou registro; SHA-256 do instalador `6367254199DEE3C1E13FFDF90CBBDB641F15D179A7755A69DF6F87F146C47950`. Runtime, parser e script executaram localmente a ajuda do importador. A leitura completa da cópia local do mesmo `bykom.sql` passou com `--allow-control-separator`: 179 clientes, 1697408 históricos lidos (histórico configurado como 0 nesta prova), hash origem `fc375f7d2db0411072b5580e5cbb5be4f34fd931c63a76bec1bc33e14ae6712f`. Sem a opção explícita, o parser recusou artefato separador 0x05 e não publicou destino. No servidor, executar a prévia com essa opção. Ainda não aplicar clientes à base ativa: as contas geradas são `BYKOM-ORDER_ID`, sem reconciliação com identificadores SG3.

## Importacao operacional reduzida r16

O r16 substitui r15; SHA-256 `6A1DAD7553559AB1BFCA19AC1343E745560279FD316D3024B7FF409CF4D28A87`. A prévia inclui somente cliente, endereço, contatos, zonas, sensores, receptor, conta e partição. Não publica `LegacyHistory`, históricos, senhas, operadores, ocorrências antigas ou comandos SQL. A leitura completa validada do dump produziu 179 clientes, 0 históricos, hash fonte `fc375f7d2db0411072b5580e5cbb5be4f34fd931c63a76bec1bc33e14ae6712f`. Usa `--allow-control-separator` porque o dump real contém artefato 0x05 entre tuplas; o relatório registra isso. Próxima ação no servidor: instalar r16 e gerar a prévia isolada. Aplicar os 179 clientes à base ativa permanece dependente da reconciliação explícita das contas SG3.
